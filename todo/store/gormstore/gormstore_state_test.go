...
func TestSetTaskState_Progressing(t *testing.T) {
	...
	updated, err := s.SetTaskState(task.ID, model.StateProgressing)
	if err != nil {
...
func TestSetTaskState_BlockedReturnsError(t *testing.T) {
	...
	_, err := s.SetTaskState(task.ID, model.StateBlocked)
...
func TestSetTaskState_ArchivedReturnsError(t *testing.T) {
	s := newTestStore(t)
	task, _ := s.CreateTask("Task", "", 0, nil, nil)
	s.ArchiveTask(task.ID, true)

	_, err := s.SetTaskState(task.ID, model.StateProgressing)
...
func TestSetTaskState_ClearsBlockerEntries(t *testing.T) {
	...
	_, err := s.SetTaskState(b.ID, model.StateProgressing)
	if err != nil {
		t.Fatalf("set state: %v", err)
	}

	detail, _ := s.GetTask(b.ID)
...
func TestSetTaskState_DoneCascadeUnblocks(t *testing.T) {
	...
	_, err := s.SetTaskState(a.ID, model.StateDone)
	if err != nil {
...
func TestAddBlockers_Basic(t *testing.T) {
	...
	result, err := s.AddBlockers(b.ID, []uint{a.ID})
	if err != nil {
...
func TestAddBlockers_MultipleBlockers(t *testing.T) {
	...
	s.AddBlockers(c.ID, []uint{a.ID, b.ID})

	// Complete A — C should still be Blocked (B still blocks it)
	s.SetTaskState(a.ID, model.StateDone)

	detail, _ := s.GetTask(c.ID)
...
func TestAddBlockers_SelfBlock(t *testing.T) {
	...
	_, err := s.AddBlockers(a.ID, []uint{a.ID})
	if err == nil {
...
func TestAddBlockers_BlockByDone(t *testing.T) {
	...
	_, err := s.AddBlockers(b.ID, []uint{a.ID})
	if err == nil {
...
func TestAddBlockers_CycleDetection(t *testing.T) {
	...
	_, err := s.AddBlockers(a.ID, []uint{b.ID})
	if err == nil {
...
func TestRemoveBlockers_AutoUnblocks(t *testing.T) {
	...
	result, err := s.RemoveBlockers(b.ID, []uint{a.ID})
	if err != nil {
...
func TestRemoveBlockers_NonBlockedIsNoop(t *testing.T) {
	...
	_, err := s.RemoveBlockers(a.ID, []uint{999})
	if err != nil {
